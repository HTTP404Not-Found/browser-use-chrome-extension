...
//   { type: 'listIframes' }
//   { type: 'getSelection' }
//   { type: 'readTerminal' }    -> text of the focused / visible terminal
//   { type: 'terminalText' }    -> terminal text, or page body as a fallback
//   { type: 'echoProbe' }       -> tail of whatever typed input landed in
//   { type: 'focusTerminal' }
...
//   Ace-based terminals    -> AWS CloudShell renders into an Ace editor's
//                             .ace_text-layer.
//
// Pages often hold several terminals (one per session tab, plus hidden status
// spans that reuse the .terminal class). Reading "the block with the most
// text" picked a stale session, so we read the one holding focus, else the
// last visible one. term.js also pads every row with spaces, which made a
// tail read pure whitespace, so trailing blanks are stripped.
...
function pickTerminalElement(nodes) {
  const list = Array.from(nodes);
  const active = document.activeElement;
  if (active && active !== document.body) {
    // Document order puts ancestors first, so the last hit is the innermost.
    const holding = list.filter((el) => el === active || el.contains(active));
    if (holding.length) return holding[holding.length - 1];
  }
  const visible = list.filter((el) => isVisible(el) && (el.innerText || '').trim());
...
function readTerminal() {
  ...
  if (xterms.length) {
    const xterm = pickTerminalElement(xterms) || xterms[xterms.length - 1];
    const rows = xterm.querySelector('.xterm-rows');
    if (rows && (rows.innerText || '').trim()) {
      return { ok: true, renderer: 'xterm-dom', readable: true, text: trimTerminalText(rows.innerText) };
    }
    const acc = xterm.querySelector('.xterm-accessibility');
    if (acc && (acc.innerText || '').trim()) {
      return { ok: true, renderer: 'xterm-accessibility', readable: true, text: trimTerminalText(acc.innerText) };
    }
  ...
  if (aces.length) {
    const ace = pickTerminalElement(aces);
    const layer = ace?.querySelector('.ace_text-layer');
    if (layer) {
      const isTerminal = !!ace.closest('[id*="term" i], [class*="term" i]');
      return {
        ok: true,
        renderer: isTerminal ? 'ace-terminal' : 'ace-editor',
        readable: true,
        text: trimTerminalText(layer.innerText || '')
      };
    }
  }

  const term = pickTerminalElement(document.querySelectorAll('.terminal, .terminal-container'));
  if (term) {
    return { ok: true, renderer: 'termjs', readable: true, text: trimTerminalText(term.innerText || '') };
  }
...
function echoProbe() {
  ...
  const terminalInput = !!el?.classList && (el.classList.contains('xterm-helper-textarea') || el.classList.contains('ace_text-input'));
  if (el && (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') && !terminalInput) {
    return { ok: true, source: 'field', readable: true, sample: String(el.value || '').slice(-400) };
  }
  if (el && el.isContentEditable) {
    return { ok: true, source: 'contenteditable', readable: true, sample: (el.innerText || '').slice(-4000) };
  }
  const term = readTerminal();
  if (term.readable) {
    return { ok: true, source: term.renderer, readable: true, sample: term.text.slice(term.renderer === 'ace-editor' ? -4000 : -400) };
  }
  ...
  const body = trimTerminalText(document.body?.innerText || '').slice(-400);
  return { ok: true, source: 'body', readable: !!body.trim(), sample: body };
}

// Full text of the terminal (or the page body as a last resort) so the
// background can capture what a command printed after Enter.
function terminalText() {
  const term = readTerminal();
  if (term.readable) return { ok: true, readable: true, renderer: term.renderer, text: term.text.slice(-20000) };
  if (term.renderer && term.renderer !== 'none') return { ok: true, readable: false, renderer: term.renderer };
  const body = trimTerminalText(document.body?.innerText || '');
  return { ok: true, readable: !!body, renderer: 'body', text: body.slice(-20000) };
}

// ---------------------------------------------------------------------------
// Focus the real terminal input
// ---------------------------------------------------------------------------
// term.js / tty.js render output to a visible .terminal DIV but accept keystrokes
// on a HIDDEN <textarea> positioned off-screen. Clicking the visible DIV does
// not focus the textarea — tty.js's own click handler does, but only if the
// click lands on the scroll/buffer area, not on empty prompt text. We try a
// cascade of strategies and verify focus actually landed on a writable input,
// because CDP Input.dispatchKeyEvent silently no-ops if the wrong element has
// focus, and the LLM cannot tell the difference between "key arrived but
// renderer is slow" and "focus is wrong, nothing will ever arrive".

function focusTerminal() {
  const candidates = [];
  const add = (el, why) => {
    if (el && !candidates.some((c) => c.el === el)) candidates.push({ el, why });
  };
  // Candidates come from the terminal that holds focus or is visible, not from
  // any matching element on the page — stale session tabs reuse the classes.
  const xterm = pickTerminalElement(document.querySelectorAll('.xterm'));
  if (xterm) add(xterm.querySelector('.xterm-helper-textarea'), 'xterm-helper-textarea');
  const ace = pickTerminalElement(document.querySelectorAll('.ace_editor'));
  if (ace) add(ace.querySelector('textarea.ace_text-input'), 'ace-text-input');
  const term = pickTerminalElement(document.querySelectorAll('.terminal'));
  if (term) {
    add(term.querySelector('textarea'), 'terminal-textarea');
    add(term.querySelector('[contenteditable="true"]'), 'contenteditable');
    // term.js takes keystrokes on the focused .terminal DIV itself.
    if (term.hasAttribute('tabindex')) add(term, 'terminal-div');
  }
  // Last resort: an off-screen textarea some tty.js builds use for input.
  document.querySelectorAll('textarea').forEach((t) => {
    if (!isVisible(t) && t.tabIndex !== -1) add(t, 'hidden-textarea');
  });

  // A focused term.js DIV used to count as failure, so focus_terminal returned
  // ok:false on a terminal that was working and the agent kept retrying.
  const accepts = (el) => {
    if (!el || el === document.body) return false;
    if (el.tagName === 'TEXTAREA' || el.tagName === 'INPUT' || el.isContentEditable) return true;
    return !!el.closest?.('.terminal[tabindex], .xterm') && el.tabIndex >= 0;
  };

  const tried = [];
  if (accepts(document.activeElement)) {
    tried.push('already-focused');
  } else {
    for (const c of candidates) {
      try {
        c.el.focus({ preventScroll: true });
        tried.push(c.why);
        if (document.activeElement === c.el) break;
      } catch {}
    }
  }

  const focused = document.activeElement;
  const ok = accepts(focused);
  return {
    ok,
    activeTag: focused?.tagName?.toLowerCase() || null,
    activeClass: focused ? String(focused.className || '').slice(0, 100) || null : null,
    activeId: focused?.id || null,
    isContentEditable: !!focused?.isContentEditable,
    tried,
    hint: ok
      ? undefined
      : 'No terminal input could be focused. The terminal may not have launched yet (click Launch Terminal once and wait a few seconds, then retry), or it uses a renderer that draws to <canvas> (no DOM input will ever work — see browser_read_terminal renderer=xterm-canvas).'
  };
}
    ...
    try {
      switch (msg.type) {
        ...
        case 'echoProbe':
          ...
          break;
        case 'terminalText':
          sendResponse(terminalText());
          break;
        case 'focusTerminal':
          sendResponse(focusTerminal());
          break;
        case 'sendKeys':
